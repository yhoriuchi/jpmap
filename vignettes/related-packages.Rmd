...
- explicit disputed-territory handling: included quietly by default, removable
  by the user, and highlightable when needed.
...
`jpmap` includes disputed-territory shapes by default and draws them quietly
with the rest of the map. Users can remove them with
`territorial_disputes = FALSE`, include only selected areas, or highlight them
with `disputed_fill` and `disputed_dots`.
