...
saves converted GeoPackage files in the package data directory returned
by
...
smaller official MLIT N03 archive, converts it to a GeoPackage, and
saves it in
[`jpmap_data_dir()`](https://yhoriuchi.github.io/jpmap/reference/jpmap_data.md).
...
The official 2024 national source archive is about 583 MB, and the
converted GeoPackage is also large.
...
The source is MLIT National Land Numerical Information N03
...
`N03-YYYY0101_PP_GML.zip` for one-prefecture data, where `PP` is a
two-digit prefecture code.
