...
Boundary GeoPackages are large and live outside the functionality
package. Install the companion data package when you want ready-to-use
boundary files, or build the files locally from MLIT source data.
...
`jpmap` looks for boundary GeoPackages in the companion `jpmapdata`
package and in the local data directory returned by
...
You can also build boundary files locally from Japan’s official MLIT
